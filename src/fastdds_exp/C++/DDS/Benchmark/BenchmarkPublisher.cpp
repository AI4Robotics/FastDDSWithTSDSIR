...
bool BenchMarkPublisher::init(
        ...
        int size)
{
    m_iTestTimeMs = time;       // Milliseconds that the test is going to run.测试运行的总时间
    m_iTickTime = tick_time; // Milliseconds to take samples of the performance. 每隔多长时间对性能取样
    m_iWaitTime = wait_time; // Milliseconds before starting the sampling. 取样前等待时间
    m_iSize = size;          // 每个要发送数据的大小
    m_iSamplesSize = abs(m_iTestTimeMs / m_iTickTime) + 1;  // 总性能取样次数
    m_vSamples = new int[m_iSamplesSize];   // 存放每次性能取样数据
    memset(m_vSamples, 0, sizeof(int) * m_iSamplesSize); // 初始化
    m_iSamplesCount = 0;
        ...
        DataReader* reader)
{
    ...
    if (!mParent->m_bBenchmarkFinished)
    {
        switch (mParent->m_iSize)
        {
            ...
            case 0:
            {
                // BenchmarkPublisher 发送数据index(i), BenchmarkSubscriber 接收到后再回传index(i+1)
                if (reader->take_next_sample((void*)&m_Hello, &m_info) == ReturnCode_t::RETCODE_OK)
                {
                    if (m_info.valid_data)
                    {
                        ...
                        std::cout << "index of the data received: " << m_Hello.index() << std::endl;
            ...
            case 3:
            {
                if (reader->take_next_sample((void*)&m_HelloBig, &m_info) == ReturnCode_t::RETCODE_OK)
                {
                    if (m_info.valid_data)
                    {
                        ...
                        std::cout << "index of the data received: " << m_HelloBig.index() << ", m_icount" << mParent->m_iCount << std::endl;
...
void BenchMarkPublisher::runThread()
{
    ...
    while (!publish()) // 匹配到订阅者后，开始发送一个数据；否则等待
    {
        std::this_thread::sleep_for(std::chrono::milliseconds(10));
    }

    std::this_thread::sleep_for(std::chrono::milliseconds(m_iWaitTime)); // 采样前等待m_iWaitTime ms
    m_iCount = 0; // 对发送的消息计数
    ...
    while (!m_bBenchmarkFinished)
    {
        ...
        if (elapsed.count() > m_iTestTimeMs) // 时间超过设置的测试总时长，终止测试
        {
        ...
        else
        {
            ...
            {
                m_vSamples[m_iSamplesCount++] = m_iCount - iPrevCount; // 每个采样周期（trick_time）内传递的数据个数
                iPrevCount = m_iCount;
            }
