...
MixedFlowControlSubscriber::MixedFlowControlSubscriber(std::string data_path)
    ...
    , myType_2(new MsgRGBImagePubSubType())
...
bool MixedFlowControlSubscriber::init()
{
    ...
    topic_2 = participant_->create_topic("MsgRGBImagePubSubTopic", myType_2.get_type_name(), TOPIC_QOS_DEFAULT);
    ...
    reader_2 = subscriber_->create_datareader(topic_2, rqos, &m_listener);
        ...
        DataReader* reader)
{
    ...
    if (reader->take_next_sample(&st_2, &info) == ReturnCode_t::RETCODE_OK)
    {
        if (info.valid_data)
        {
            ++received_num;
            // 设置数据接收时间
	    start_time.seconds(st_2.msg_start_seconds());
            start_time.nanosec(st_2.msg_start_nanosec());
            // 设置数据接收时间
            Time_t::now(now_time);
            int i = 0;
            ...
            st_2.msg_end_seconds(now_time.seconds());
            st_2.msg_end_nanosec(now_time.nanosec());
            ...
            */
            ...
            statisticsOut << received_num << ", " << st_2.msg_sequence_num() << ", " << st_2.msg_type() 
                << ", " << st_2.msg_priority() << ", " << start_time << ", " << now_time << "," << during_time << std::endl;
            std::cout << "###Sample received NO." << received_num << "\t type.msg_sequence_num: " << st_2.msg_type() << "." << st_2.msg_sequence_num() 
                << "\t msg_priority: " << st_2.msg_priority() << "\t msg_trans_time: " << during_time << "ns ***" << std::endl;
        }
        return;
    }

    // std::cout << ".....reader的类型: " << reader->type() << "type:" << (reader->type() == TypeSupport(new MsgImagePubSubType())) << std::endl;
}
