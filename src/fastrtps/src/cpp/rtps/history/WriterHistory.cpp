        ...
        WriteParams& wparams)
{
    ...
    std::cout << "==WriterHistory::prepare_and_add_change(CacheChange_t* a_change, WriteParams& wparams): change添加到History中" <<
        ", 当前History的大小为: " << m_changes.size() << "==" << std::endl;

    if (static_cast<int32_t>(m_changes.size()) == m_att.maximumReservedCaches)
    {
        std::cout << "==History 已满==" << std::endl;
        m_isHistoryFull = true;
    }

    logInfo(RTPS_WRITER_HISTORY,
            "Change " << a_change->sequenceNumber << " added with " << a_change->serializedPayload.length << " bytes");
    std::cout << "==Change " << a_change->sequenceNumber << " added with " << a_change->serializedPayload.length << " bytes==" << std::endl;
...
bool WriterHistory::remove_min_change()
{
    ...
    std::lock_guard<RecursiveTimedMutex> guard(*mp_mutex);
    // History队列满时，删除队列第一个元素（front）-- History处理
